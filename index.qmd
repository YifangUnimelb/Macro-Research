...
All data is obtained from the RBA. Daily or monthly data is converted to quaterly format by averaging. Following Chan(2017, other than cash rate and unemployment, variables will be transformed using log difference in BVAR. These are some standard variables to include in a macroeconomics forecast:

**Cash rate:** the cash rate target (series_id:FIRMMCRTD)

**Money aggregate(M1):** the seasonal adjusted M1 money aggregate (series_id:DMAM1S)

**Money aggregate(M3):** the seasonal adjusted M3 money aggregate (series_id:DMAM3S)
...
ggplot(cr, aes(x = quarter, y = average)) +
  geom_line() + # Creates the time series line
  labs(x = "Year", y = "CPI", title = "Time Series Plot") +
